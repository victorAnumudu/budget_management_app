...
const validationSchema = addPVFieldsValidation // Yup.object().shape({}) //
...
const AddPVCom = memo(() => {
    ...
    const getDetails = useMutation({
        mutationFn: (fields) => {
            return getAnEconomicItem(fields)
        },
        onSuccess: (res) => {
    ...
    const economicItem = getDetails?.data?.data?.data?.economic_item // ECONOMIC ITEM LIST
    ...
    const addPV = useMutation({
        ...
        onSuccess: (res) => {
            ...
            queryClient.refetchQueries({queryKey: [...queryKeys.dashboardData]})
            queryClient.refetchQueries({queryKey: [...queryKeys.dashboardSummaryData]})
            queryClient.refetchQueries({queryKey: [...queryKeys.dashboardRightPanelData]})
        },
        onSettled: () => {
            setTimeout(()=>{
                addPV.reset()
    ...
    return (
        <>
            <div className='w-full flex flex-col gap-8'>
                ...
                {economicCode && economicItem && economicItem.length > 0 ?
                    <div className='w-full'>
                        <button className='px-2 py-1 mb-2 bg-primary rounded-md text-white-aside' onClick={()=>getDetails.reset()}>Back</button>
                                            ...
                                            <div className='px-2 py-2 mb-4 flex flex-col flex-wrap gap-4 lg:gap-8'>
                                                {/* <div className=''>
                                                    <h2 className='text-2xl text-center'>
                                                        Current Bal: 
                                                        <span className='font-semibold'>{formatNumber('')}</span>
                                                    </h2>
                                                    <h2 className='text-2xl text-center'>
                                                ...
                                                <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-8'>
                                                    <div className='relative text-input flex flex-col gap-1'>
                                                        <p className='text-sm font-semibold dark:text-slate-high'>
                                                            Date Captured <span className='text-red-500 text-10'>{(props.errors.date_captured && props.touched.date_captured) ? props.errors.date_captured : ''}</span>
                                                        </p>
                                                        <InputText 
                                                            id='date_captured' 
                                                            type='date' 
                        ...
                        <div className='grid grid-cols-1 xxs:grid-cols-2 gap-4 place-items-end'>
                            <div className='w-full relative text-input flex flex-col gap-1'>
                                ...
                                <InputText 
                                    ...
                                    name='code' 
                                    value={economicCode}
                                    handleChange={(e)=>setEconomicCode(e.target.value)}
                                />
                            </div>
                            <MainBtn 
                                type='button'
                                disabled={getDetails.isPending || !economicCode} 
                                loading={getDetails.isPending}
                                onClick={()=>{getDetails.mutate({economic_code: economicCode})}}
                                className={`bg-primary dark:bg-primary-dark px-2 py-1 mt-4 rounded-md text-white font-medium sm:self-end`}
                                text='Search'
                            />
                        </div>
                        {getDetails?.isError && <p className='text-red-500 text-center p-3'>{getDetails?.error?.message}</p>}
            ...
            { (addPV.isError || addPV.isSuccess || addPV.isPending) &&
                <StatusModal 
                    ...
                    cLoseModal={()=>{setEconomicCode(''); addPV.reset()}}
                />
            }
        </>
    )
