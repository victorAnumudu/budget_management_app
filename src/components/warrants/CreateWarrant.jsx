...
const CreateWarrant = memo(({budget_type}) =>{
    ...
    const CreateCapWarrant = useMutation({
        mutationFn: (fields) => {
    ...
    const proceed = () => {
        if(state){
            ...
            return proceedToCreateCapWarrant()
        }
    }
    ...
    return (
        <>
                                                ...
                                                <tr key={item.id || index} className="border-t border-dashed border-slate-high">
                                                    ...
                                                    <td className="p-2">
                                                        <div className='w-full flex items-center gap-2 whitespace-nowra'>
                                                            ...
                                                            <div className="text-left">
                                                                <div title={item?.mda_info?.mda_name} className="text-sm font-semibold line-clamp-1">{item?.beneficiary_mda}</div>
            ...
            { actionModal.name == 'status' &&
                <StatusModal 
                    isPending={CreateCapWarrant.isPending || addMoreToWarrantMutation.isPending}
                    text={(CreateCapWarrant.isSuccess || addMoreToWarrantMutation.isSuccess) ? 'Warrant added, preceed to generate the warrant' : 'Unable to add'} 
                    isSuccess={CreateCapWarrant.isSuccess || addMoreToWarrantMutation.isSuccess}
                    cLoseModal={()=>{closeActionModal()}}
                />
            }
        </>
